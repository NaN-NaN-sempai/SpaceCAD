...
class JSDoc {
    ...
    static Type = class Type {
        ...
        constructor(content, types = [], description = null) {
            ...
            const typeFind = types.find(instance =>
                typeof content == "object" ? instance.type == content.type :
                    instance.type == type());
            if (typeFind) {
                if (typeof content == "object")
                    ...
                    if (description)
                        typeFind.description = description;
            }
            ...
            if (typeof content == "object") {
                ...
                if (this.extends) {
                    this.extends =
                        JSDoc.Type.instances.find(instance => instance.type == this.extends) ||
                        this.extends;
                }
        ...
        register(types = []) {
            if (!types.some(item => item.type === this.type)) {
                types.push({
                    ...this
                });
            }
    ...
    static UseType = class UseType extends JSDoc.Type {
            ...
            for (let [index, line] of entries) {
                ...
                if (!defining) {
                    if (line.startsWith("@type")) {
                        obj.type = line.replace("@type", "").trim();
                        defining = true;
                    }
                } else {
                    if (line === "@end") {
...
export const objectify = (dir = "./", ignorePath, ignoreStartsWith, fileTypes, types = []) => {
    if (fs.statSync(dir).isFile()) return new File(dir, types);
